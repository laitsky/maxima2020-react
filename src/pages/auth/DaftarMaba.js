...
const DaftarMaba = () => {
  const {
    register, handleSubmit, reset, errors, watch,
  } = useForm();
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <AlignMiddle>
          ...
          <MxmInput
            ...
            name="nim"
            placeholder="NIM"
            ref={register({ required: 'Isi Nomor Induk Mahasiswa kamu' })}
          />
          ...
          <MxmInput
            ...
            ref={register({
              validate: (val) => val === password.current || 'Kata sandi tidak sama',
            })}
          />
          { errors.password_conf && <Error>{errors.password_conf.message}</Error> }
          <MxmButton mt="1.5em">Daftar</MxmButton>
          <Box mt={4}>
            <Link to="/login" exact style={{ textDecoration: 'none' }}>
              <h4 className="mxm-cyan">
                Sudah memiliki akun?
                <span style={{ fontFamily: 'canaro-bold', marginLeft: 5 }}>Masuk</span>
              </h4>
            </Link>
          </Box>
        </AlignMiddle>
